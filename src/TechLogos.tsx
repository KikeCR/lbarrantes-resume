	...
	<div className="mt-8">
		...
		<div className="mt-4 grid grid-cols-2 gap-6 sm:grid-cols-3 lg:gap-8">
			{logos.map(({ title, Icon }) => (
				<div
					key={title}
					className="group flex flex-col items-center gap-4 rounded-card border border-card-border bg-card py-10 text-center transition-all duration-300 hover:-translate-y-0.5 hover:border-secondary/50 lg:py-14"
				>
					<span className="flex h-14 w-14 items-center justify-center rounded-full bg-chip text-2xl text-secondary transition-colors group-hover:bg-secondary/20 lg:h-16 lg:w-16 lg:text-3xl">
						<Icon />
					</span>
		...
		>
			{repeatedLogos.map(({ title, Icon }, i) => (
				<div
					...
					className="flex shrink-0 items-center gap-2 rounded-full border border-card-border bg-card px-4 py-2"
				>
					<Icon size={16} />
